...
wait_database_started ()
{
    ...
    mysql=( mysql -h db -u$1 -p$2 --ssl-mode=DISABLED )
...
if [ $MW_AUTOUPDATE == 'true' ]; then
    ...
    #     'set -x; cd $MW_HOME/extensions/cldr && curl -LO http://www.unicode.org/Public/cldr/latest/core.zip && unzip -q core.zip -d core && php rebuild.php && set +x;'
